...
//--------------------------------------------------------------
void ofxOBDSim::open(string p, ofxOBDCar * c){
	car = c;

	ss = new struct simsettings;
	obdsim_initialisesimsettings(ss);

	sp = new WindowsSimPort(p.c_str());
...
//--------------------------------------------------------------
void ofxOBDSim::close(){
	delete ss;
	delete sp;
}

//--------------------------------------------------------------
int ofxOBDSim::parse_ATcmd(struct simsettings *ss, OBDSimPort *sp, char *line, string& response) {
...
//--------------------------------------------------------------
int ofxOBDSim::parse_01cmd(struct simsettings *ss, OBDSimPort *sp, char *line, string& response) {
	// This is an AT command
	...
	if(0 == strncmp(at_cmd, "0D", 2)) { //SPEED //A
		cout << "SPEED " << car->speed << endl;
		command_recognised = 1;
		unsigned char A = car->speed;
		response = "41 0D " + ofToHex(A);
	}
	
	else if(0 == strncmp(at_cmd, "0C", 2)) { //RPM // ((A*256)+B)/4
		cout << "RPM "<< car->rpm << endl;
		command_recognised = 1;
		unsigned char A = (car->rpm*4)/256;
		unsigned char B = car->rpm*4;
		response = "41 0C " + ofToHex(A) + " " + ofToHex(B);
	}
	
	else if(0 == strncmp(at_cmd, "1F", 2)) { //RUNTIME //long
		cout << "RUNTIME " << car->runtime << endl;
		command_recognised = 1;
		unsigned char A = car->runtime/256;
		unsigned char B = car->runtime;
		response = "41 1F " + ofToHex(A) + " " + ofToHex(B);
	}

	else if(0 == strncmp(at_cmd, "2F", 2)) { //NAFTA
		cout << "NAFTA " << car->nafta << endl;
		command_recognised = 1;
		unsigned char A = 255*(car->nafta/100.0f);
		response = "41 2F " + ofToHex(A);
	}

	else if(0 == strncmp(at_cmd, "05", 2)) { //TEMP //C
		cout << "TEMP " << car->temp << endl;
		command_recognised = 1;
		unsigned char A = car->temp+40;
		response = "41 05 " + ofToHex(A);
	}
